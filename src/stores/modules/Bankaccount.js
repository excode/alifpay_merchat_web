    ...
    state:() => ({
      ...
      actions: {
        async getBankaccount(request = {}) {
            this.loading =true 
            const response = await getData( '/bankaccount',request);
          ...
          async getBankaccountAll(request = {}) {
            this.loading =true 
            const response = await getData( '/bankaccount/all',request);
            if(response["errors"]==undefined) {
              this.bankaccounts = response
              this.loading = false
              this.error = null
          ...
          async getBankaccountSuggestions(request = {}) {
            this.loading =true 
            const response = await getData( '/bankaccount/suggestions',request);
            if(response["errors"]==undefined) {
          ...
          async getBankaccountDetails (Id = '') {
            this.loading = true
            const response = await getData('/bankaccount/'+ Id);
          ...
          async addBankaccount (request = {}) {
            ...
            const response = await  postData( '/bankaccount', request )
            if(response["errors"]==undefined || response["errors"]==null) {
          ...
          async  updateBankaccount (request = {}) {
            ...
            delete request.id
            this.curdLoading =true 
            const response = await  patchData( '/bankaccount/'+ id, request )
            if(response["errors"]==undefined || response["errors"]==null) {
          ...
          async deleteBankaccount (id) {
            this.loading =true 
            const response = await  deleteData( '/bankaccount/'+ id )
            if(response["errors"]==undefined || response["errors"]==null) {
